...
void TestMine1() {
  ...
  const Ini::Section expected_harry = {
    {"a", "b"},
    {"avada", "kedavra"}
  };

  ASSERT_EQUAL(doc.GetSection("Harry"), expected_harry);

  const Ini::Section expected_second = {
    {"a", "b"},
    {"b", "a"}
  };

  ASSERT_EQUAL(doc.GetSection("second"), expected_second);
}
...
void TestMineEmptySection() {
  ...
  const Ini::Section expected_third = {};
  ASSERT_EQUAL(doc.GetSection("third"), expected_third);
}

void TestMineEmptyDocument() {
  istringstream input("");

  Ini::Document doc = Ini::Load(input);
  ASSERT_EQUAL(doc.SectionCount(), 0);
}

void TestMineEmptyDocumentWithNewlines() {
  istringstream input(
    "\n\n\n\n"
  );
...
int main() {
  ...
  RUN_TEST(tr, TestMine0);
  RUN_TEST(tr, TestMine1);
  RUN_TEST(tr, TestMine2);
  RUN_TEST(tr, TestMineEmptySection);
  RUN_TEST(tr, TestMineEmptyDocument);
  RUN_TEST(tr, TestMineEmptyDocumentWithNewlines);
  return 0;
}
